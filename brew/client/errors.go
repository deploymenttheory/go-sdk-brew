...
// APIError represents an error response from the Homebrew API.
...
func (e *APIError) Error() string {
	if e.Code != "" {
		return fmt.Sprintf("Homebrew API error (%d %s) [%s] at %s %s: %s",
			e.StatusCode, e.Status, e.Code, e.Method, e.Endpoint, e.Message)
	}
	return fmt.Sprintf("Homebrew API error (%d %s) at %s %s: %s",
		e.StatusCode, e.Status, e.Method, e.Endpoint, e.Message)
}

// brewErrorBody is a common shape for Homebrew API error responses.
type brewErrorBody struct {
